...
interface ImageItem extends ImageMeta {
  selected: boolean;
}

function getFormat(contentType: string, url: string): string {
  const ct = contentType?.match(/image\/(\w+)/);
  if (ct) { const f = ct[1].toLowerCase(); return f === 'jpeg' ? 'jpg' : f; }
  const urlM = url.match(/\.(jpe?g|png|gif|webp|svg|avif|bmp)/i);
  if (urlM) return urlM[1].toLowerCase().replace('jpeg', 'jpg');
  return 'img';
}
...
function App() {
  ...
  const handleFetch = async (e?: React.FormEvent) => {
    ...
    try {
      const { sessionId: sid, images: metas } = await fetchImages(trimmed);
      setSessionId(sid);
      setImages(metas.map(m => ({ ...m, selected: false })));
      if (metas.length === 0) setError('No images found on this page.');
    } catch (err) {
  ...
  const handleDownloadSingle = async (imgId: number) => {
    if (!sessionId) return;
    setDownloadingIds(prev => new Set(prev).add(imgId));
    try {
      await downloadImage(sessionId, imgId);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Download failed');
    } finally {
      setDownloadingIds(prev => { const n = new Set(prev); n.delete(imgId); return n; });
    }
  };
  ...
  const handleDownloadSelected = async () => {
    ...
    try {
      await downloadSelectedImages(sessionId, ids);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Download failed');
    } finally {
  ...
  const handleDownloadAll = async () => {
    ...
    try {
      await downloadAllImages(sessionId);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Download failed');
    } finally {
  ...
  return (
    <div className="min-h-screen bg-[#f4f6f9]">
          ...
          <form onSubmit={handleFetch} className="flex-1 flex gap-2">
            ...
            <button type="submit" disabled={loading || !inputUrl.trim()}
              className={`px-5 py-2.5 rounded-xl text-sm font-semibold text-white transition-colors flex items-center gap-2 shrink-0
                ${loading || !inputUrl.trim() ? 'bg-gray-300 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700 active:bg-blue-800'}`}>
              {loading
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        ...
        {/* Loading state — Puppeteer takes ~10-30s */}
        {loading && (
          <div className="flex flex-col items-center justify-center py-32 gap-4">
            <RefreshCw className="w-10 h-10 text-blue-500 animate-spin" />
            <div className="text-center">
              <p className="text-gray-700 font-medium">Opening browser and extracting images…</p>
              <p className="text-sm text-gray-400 mt-1">This may take up to 30 seconds</p>
            </div>
            <div className="w-full max-w-2xl grid grid-cols-4 gap-3 mt-4 opacity-40">
          ...
          <>
            ...
            <div className="mb-4 flex flex-wrap items-center gap-2">
              <div className="text-sm font-semibold text-gray-700 mr-1">
                ...
                {sourceHostname && <span className="font-normal text-gray-400 ml-1.5">— {sourceHostname}</span>}
              </div>
              ...
              <div className="flex items-center gap-2 ml-auto flex-wrap">
                ...
                {selectedCount > 0 && (
                  <button onClick={handleDownloadSelected} disabled={downloadingSelected || downloadingAll}
                    ...
                    {downloadingSelected
                      ...
                      : <><Archive className="w-3.5 h-3.5" />ZIP {selectedCount} selected</>}
                  </button>
                )}

                <button onClick={handleDownloadAll} disabled={downloadingAll || downloadingSelected}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
              {images.map((img, idx) => {
                const isDown = downloadingIds.has(img.id);
                const fmt = getFormat(img.contentType, img.originalUrl);

                return (
                  <div key={img.id} onClick={() => toggleSelect(idx)}
                    className={`relative group rounded-xl overflow-hidden cursor-pointer transition-all bg-gray-200
                      ${img.selected ? 'ring-2 ring-offset-2 ring-blue-500 shadow-md' : 'hover:ring-2 hover:ring-offset-1 hover:ring-gray-300'}`}>

                    {/* Image served from session cache — no expiry issues */}
                    <div className="aspect-square">
                      <img
                        src={getImagePreviewUrl(sessionId, img.id)}
                        alt=""
                        loading="lazy"
                    ...
                    <div className={`absolute top-2 left-2 transition-opacity
                      ...
                      <div className={`w-5 h-5 rounded flex items-center justify-center shadow border
                        ${img.selected ? 'bg-blue-500 border-blue-500' : 'bg-white/80 border-gray-300'}`}>
                        {img.selected && (
...
function App() {
  ...
  const handleFetch = async (e?: React.FormEvent) => {
    ...
    let trimmed = inputUrl.trim();
    if (!trimmed) return;
    // Auto-prefix https:// so bare domains like "google.com" work
    if (!/^https?:\/\//i.test(trimmed)) trimmed = 'https://' + trimmed;
              ...
              <div className="flex items-center gap-2 ml-auto flex-wrap">
                ...
                {selectedCount > 0 && (
                  <button
                    ...
                    disabled={downloadingSelected || downloadingAll}
                    className="text-xs px-3 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white flex items-center gap-1.5 disabled:opacity-50 transition"
                  >
                    {downloadingSelected
                      ? <><RefreshCw className="w-3.5 h-3.5 animate-spin" />Zipping…</>
